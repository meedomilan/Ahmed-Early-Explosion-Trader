...
EARLY_PRESSURE_SCORE = float(os.getenv("EARLY_PRESSURE_SCORE", "50"))
READY_PRESSURE_SCORE = float(os.getenv("READY_PRESSURE_SCORE", "60"))
ENTRY_PRESSURE_SCORE = float(os.getenv("ENTRY_PRESSURE_SCORE", "70"))
MIN_PRESSURE_FACTORS = int(os.getenv("MIN_PRESSURE_FACTORS", "2"))
MAX_EARLY_EXTENSION_ATR = float(os.getenv("MAX_EARLY_EXTENSION_ATR", "0.85"))
MAX_ENTRY_EXTENSION_ATR = float(os.getenv("MAX_ENTRY_EXTENSION_ATR", "0.65"))
PRESSURE_HISTORY = int(os.getenv("PRESSURE_HISTORY", "12"))
PRESSURE_ACCEL_MIN = float(os.getenv("PRESSURE_ACCEL_MIN", "4.0"))
VWAP_RECLAIM_SCORE = float(os.getenv("VWAP_RECLAIM_SCORE", "58"))
BB_COMPRESSION_SCORE = float(os.getenv("BB_COMPRESSION_SCORE", "58"))
FAILED_BREAK_SCORE = float(os.getenv("FAILED_BREAK_SCORE", "60"))
VOL_EXPANSION_SCORE = float(os.getenv("VOL_EXPANSION_SCORE", "60"))
MOMENTUM_SHIFT_SCORE = float(os.getenv("MOMENTUM_SHIFT_SCORE", "58"))
...
def session_vwap(rows):
    ...
    vol = 0.0
    for row in rows:
        h, l, c, v = float(row[2]), float(row[3]), float(row[4]), float(row[5])
        tp = (h + l + c) / 3
        pv += tp * v
        vol += v
...
def market_pressure(direction, k1m, k3m, k15, c15, m1, m3, ind1, ind3, ind15, ob, oi15, fund, smart, zone):
    price = m1.get("price", 0)
    vwap1 = session_vwap(k1m[-60:])

    if direction == "BUY":
        vwap_reclaim = price > vwap1 and float(k1m[-2][4]) <= vwap1
        failed_break = float(k1m[-1][3]) < min(float(x[3]) for x in k1m[-8:-1]) and float(k1m[-1][4]) > min(float(x[3]) for x in k1m[-8:-1])
    else:
        vwap_reclaim = price < vwap1 and float(k1m[-2][4]) >= vwap1
        failed_break = float(k1m[-1][2]) > max(float(x[2]) for x in k1m[-8:-1]) and float(k1m[-1][4]) < max(float(x[2]) for x in k1m[-8:-1])

    liquidity = clamp(
        45 + max(0, oi15.get("oi_change", 0))*12
        + (8 if m1.get("sweep") or m3.get("sweep") else 0)
        + (7 if ob.get("absorption", 0) >= 55 else 0)
        + (5 if fund.get("funding_score", 50) >= 55 else 0)
    )
    execution = clamp(
        0.34*m1.get("delta_accel", 50)
        + 0.18*m3.get("delta_accel", 50)
        + 0.18*ob.get("imbalance", 50)
        + 0.16*c15.get("cvd_strength", 50)
        + 0.14*smart.get("smart_score", 50)
    )
    price_pressure = clamp(
        0.34*c15.get("compression", 50)
        + 0.22*(75 if ind15.get("bb_touch") else 45)
        + 0.20*(78 if vwap_reclaim else 45)
        + 0.14*(75 if ind1.get("volume_expansion") else 45)
        + 0.10*c15.get("breakout", 50)
    )
    structure = clamp(
        0.28*zone.get("zone_score", 50)
        + 0.18*(80 if zone.get("ob_distance_atr", 99) <= 0.9 else 40)
        + 0.16*(80 if zone.get("fvg_distance_atr", 99) <= 0.8 else 40)
        + 0.18*(80 if m1.get("micro_break") else 40)
        + 0.20*(82 if failed_break else 40)
    )
    time_pressure = clamp(
        0.34*smart.get("persistence", 50)
        + 0.24*smart.get("delta_persistence", 50)
        + 0.22*smart.get("book_persistence", 50)
        + 0.20*smart.get("oi_accel", 50)
    )

    total = clamp(0.22*liquidity + 0.28*execution + 0.20*price_pressure + 0.18*structure + 0.12*time_pressure)

    strategies = {
        "VWAP_RECLAIM": clamp((24 if vwap_reclaim else 0)+(18 if m1.get("delta_flip") else 0)+(14 if m1.get("rejection") else 0)+(12 if ob.get("imbalance",50)>=56 else 0)+20),
        "BB_COMPRESSION": clamp((24 if c15.get("compression",0)>=58 else 0)+(18 if ind15.get("bb_touch") else 0)+(16 if oi15.get("oi_change",0)>0.05 else 0)+(14 if m1.get("delta_accel",50)>=55 else 0)+18),
        "FAILED_BREAK": clamp((30 if failed_break else 0)+(18 if m1.get("sweep") else 0)+(16 if m1.get("rejection") else 0)+(14 if m1.get("delta_flip") else 0)+14),
        "VOLATILITY_EXPANSION": clamp((22 if c15.get("compression",0)>=55 else 0)+(22 if ind1.get("volume_expansion") else 0)+(16 if oi15.get("oi_change",0)>0.05 else 0)+(16 if m1.get("delta_accel",50)>=55 else 0)+14),
        "MOMENTUM_SHIFT": clamp((20 if ind1.get("macd_turn") else 0)+(16 if ind1.get("rsi_turn") or ind1.get("rsi_divergence") else 0)+(18 if m1.get("delta_flip") else 0)+(16 if m1.get("micro_break") else 0)+14),
    }
    thresholds = {
        "VWAP_RECLAIM": VWAP_RECLAIM_SCORE,
        "BB_COMPRESSION": BB_COMPRESSION_SCORE,
        "FAILED_BREAK": FAILED_BREAK_SCORE,
        "VOLATILITY_EXPANSION": VOL_EXPANSION_SCORE,
        "MOMENTUM_SHIFT": MOMENTUM_SHIFT_SCORE,
    }
    best = max(strategies, key=strategies.get)

    factors = {
        "ضغط السيولة": liquidity >= 58,
        "ضغط التنفيذ": execution >= 58,
        "Bollinger/VWAP": price_pressure >= 58,
        "الهيكل": structure >= 58,
        "استمرار الضغط": time_pressure >= 58,
    }
    ...
    return {
        ...
        "time": time_pressure,
        "strategy": best,
        "strategy_score": strategies[best],
        "strategy_threshold": thresholds[best],
        "factor_count": sum(factors.values()),
        "factors": factors,
        "extension_atr": min(m1.get("move_from_pivot_atr",99), m3.get("move_from_pivot_atr",99)),
    }


def pressure_stage(p, acceleration):
    if p["factor_count"] < MIN_PRESSURE_FACTORS:
        return None
    if p["total"] >= ENTRY_PRESSURE_SCORE and acceleration >= PRESSURE_ACCEL_MIN and p["extension_atr"] <= MAX_ENTRY_EXTENSION_ATR and p["strategy_score"] >= p["strategy_threshold"]:
        return "EXPLOSION"
    if p["total"] >= READY_PRESSURE_SCORE and p["extension_atr"] <= MAX_EARLY_EXTENSION_ATR:
        return "CONFIRMED"
    if p["total"] >= EARLY_PRESSURE_SCORE and p["extension_atr"] <= MAX_EARLY_EXTENSION_ATR:
        return "EARLY"
    return None
...
class Engine:
    def __init__(self):
        ...
        self.pressure_history: dict[tuple[str, str], deque] = {}
    ...
    async def start(self):
        ...
        if SEND_STARTUP_MESSAGE:
            await send_telegram(
                ...
                "✅ <b>Ahmed Market Pressure AI v10 بدأ العمل</b>\n\n"
                "🧭 الاستراتيجيات:\n"
                "• Trend Pullback\n"
                "• Liquidity Reversal\n"
    ...
    async def analyze_symbol(self, symbol: str) -> list[Signal]:
        ...
        for direction in ("BUY", "SELL"):
            ...
            zone = zone_context(direction, m1["price"], m1["atr"], k15, k1, k4)

            p = market_pressure(direction, k1m, k3m, k15, c15, m1, m3, ind1, ind3, ind15, ob, o15, fund, smart, zone)

            key = (symbol, direction)
            ph = self.pressure_history.setdefault(key, deque(maxlen=PRESSURE_HISTORY))
            prev = ph[-1] if ph else p["total"]
            ph.append(p["total"])
            acceleration = p["total"] - prev

            stage = pressure_stage(p, acceleration)
            if not stage:
                continue
            ...
            plan = build_trade_plan(
                direction, m1["price"], max(m1["atr"], m3["atr"]),
            ...
            reasons = [
                f"الاستراتيجية: {p['strategy']}",
                f"ضغط التنفيذ: {p['execution']:.1f}%",
                f"ضغط السعر: {p['price']:.1f}%",
                f"ضغط السيولة: {p['liquidity']:.1f}%",
                f"تسارع الضغط: {acceleration:+.1f}",
            ] + [name for name, ok in p["factors"].items() if ok]
            ...
            details = {
                ...
                "zone": zone,
                "pressure": {**p, "acceleration": acceleration, "history": list(ph)},
                "adaptive_strategy": {
                    "regime": "PRESSURE_BUILDUP",
                    "strategy": p["strategy"],
                    "strategy_score": p["strategy_score"],
                },
                "recipe": {"id": p["strategy"], "type": "MARKET_PRESSURE", "score": p["total"]},
                "reversal": {
                    "location_score": p["structure"],
                    "flow_score": p["execution"],
                    "trigger_score": p["price"],
                    "late_atr": p["extension_atr"],
                },
            }
            ...
            signals.append(Signal(
                ...
                stage=stage,
                score=p["total"],
                explosion_score=p["total"],
                entry_score=clamp(0.42*p["execution"]+0.26*p["structure"]+0.18*p["price"]+0.14*p["time"]),
                safety_score=clamp(0.30*p["liquidity"]+0.28*p["structure"]+0.22*p["time"]+0.20*(100-ob.get("spoof_risk",0))),
        ...
        if len(signals) == 2:
            ...
            if signals[0].explosion_score - signals[1].explosion_score < DIRECTION_GAP:
                return []
            signals = [signals[0]]
...
async def test_telegram():
    ...
    ok = await send_telegram(
        ...
        "✅ Telegram متصل بنجاح\n"
        "✅ Railway يعمل\n"
        "✅ Ahmed Market Pressure AI v10 جاهز\n"
        f"🕒 {now_local().strftime('%d-%m-%Y %H:%M:%S')} (السعودية)"
    )
